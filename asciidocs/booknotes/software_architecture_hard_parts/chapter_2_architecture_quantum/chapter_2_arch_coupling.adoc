...
In most architectural designs, best practices and sessions, you would be hearing a lot about "Architectural Coupling", aiming for that perfect modular architecture, and god knows how many times have you suggested, dodged, or told about "Microservice" !!

Dont get me wrong, I am not bashing the terms or the intent they are suggested over and over in our meeting rooms, but I am trying to throw some basic questions at you - * What is de-coupling in architecture ?
* How do you measure coupling in architecture ?
* When should be plan to decouple ?

In this blog entry I plan to explore these concepts, and add some metrics to it.
...
* If yes → current quantum is fine
* If no → quantum is too big

=== Cognitive load

How much of the system must you understand to make a safe change?

* If “almost everything” → quantum may be too large
* If “just my bounded context” → good fit

=== Risk containment

Does a failure or rollback in one area take down unrelated capabilities?

* If yes → quantum too large
* If no → good isolation
...
| Ops burden is already high | Don’t split yet
...
* Change A’s data model without impacting B or C
...
* Guarantee that A’s failure cannot take down B or C
...
* “Place instability where change is expected, and stability where responsibility is high.”
* Instability answers ONE question: “How easy is it for this module to change without hurting others?”
...
** UI changes shouldn’t redeploy backend
...
Good architecture doesn’t minimize quanta, coupling, or instability — it aligns them with business change.
...
* Microservices = many small quanta, not “higher quantum”
...
----
You cannot architect around coupling — only remove or accept it
----
...
* Low I → hard to change (many depend on it)

----
Instability is not bad — misplaced instability is
----

=== Some rules to follow

* Rule 1: Don’t optimize the number of quanta
...
----
Core domain → stable
Application → semi-stable
UI / adapters → unstable
----
